...
and an unpublished review of mine using unconstrained nonlinear least squares maximum likelihood estimation to derive *k* in f(x) = exp(−*k*x). I did not deem it necessary to compile a bibliography because all sources can be accessed via the respective reviews and the `DOI_original` variable. The data are structured into these variables:
